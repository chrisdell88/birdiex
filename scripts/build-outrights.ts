...
async function main() {
  ...
  // ─── No line-shopping merge for outrights — write CURRENT prices only ──
  // Outright odds are DYNAMIC: books move them throughout the tournament as
  // the leaderboard changes. A "best odds seen" merge keeps stale pre-round
  // prices that are no longer offered (e.g. Burns at +600 before R1, when
  // books now have him at +230 mid-leader). Users seeing +600 on our site
  // can't actually bet that — it's a historical artifact. So overwrite the
  // file with the fresh pull. (Matchups KEEP the line-shopping merge —
  // those odds don't move as dynamically.)
  ...
  const mergedById = new Map<number, Built>();
  for (const ne of built) mergedById.set(ne.dg_id, ne);
  ...
  console.log(`Fresh outrights snapshot: ${built.length} players with book lines.`);
